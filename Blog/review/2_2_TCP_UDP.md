...
6. **分片方式**：TCP数据大于MSS时会在TCP层将数据进⾏分片传输，到达⽬的地后同样在传输层进⾏合并，如果有某个片丢失则只需要重传丢失的分片即可；UDP数据大于MTU时会在IP层分片，同样也在⽬的IP层合并，如果某个IP分片丢失，则需要将所有分片都进⾏重传，开销大。
...
所以发送端的发送窗口是由接收窗口和拥塞窗口共同决定的，发送窗口取其中的最小值。
