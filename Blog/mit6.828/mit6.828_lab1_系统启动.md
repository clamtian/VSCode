...
> 临时页表只创建了两项，将虚拟地址的[0,4M)和[0xF0000000,0xF0000000+4M)映射到物理地址[0,4M)，这个临时页表是手写的，在 kern/entrypgdir.c 中。具体实现跟正常分页机制相似，不同的是一级页表（相当于 pgdir）只实现了两项，每一项是 4MB，核心代码如下：
...
pde_t entry_pgdir[NPDENTRIES] = {
	...
	// Map VA's [KERNBASE, KERNBASE+4MB) to PA's [0, 4MB)   PDXSHIFT=22
	[KERNBASE>>PDXSHIFT]
		= ((uintptr_t)entry_pgtable - KERNBASE) + PTE_P + PTE_W
};
```
> [0xF0000000,0xF0000000+4M)也就是[0xF0000000,0xF0400000)，其中前 10 位都是相同的，所以在[0xF0000000,0xF0400000)范围内寻址时，一级页表都会来到 [KERNBASE>>PDXSHIFT] 项，而两个一级页表项都指向同一个二级页表，所以实现了虚拟地址的[0,4M)和[0xF0000000,0xF0000000+4M)映射到物理地址[0,4M)。
