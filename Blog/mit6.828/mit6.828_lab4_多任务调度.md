...
* `sched_yield()`永远不能同时在两个cpu上运行相同的环境。它可以通过查看该环境的状态是否是ENV_RUNNING来判断该环境当前是否在某个CPU(可能是当前的CPU)上运行。
...
外部中断通常称之为 IRQ(Interrupt Requests)，IRQ到中断描述符表的入口不是固定的。不过在 `pic_init` 中我们将IRQ的0-15映射到了IDT的[IRQ_OFFSET, IRQ_OFFSET+15]。其中IRQ_OFFSET为32，所以IRQ在IDT中范围为[32, 47]，共16个。JOS中对中断做了简化处理，在内核态时外部中断是禁止的，在用户态时才会开启。中断开启和禁止是通过eflags寄存器的 FL_IF 位来控制，为1表示开启中断，为0则禁止中断。

在`inc/trap.h`，IRQ_OFFSET被定义为十进制数32。因此，IDT条目32-47对应于IRQs 0-15。例如，时钟中断是IRQ 0。因此IDT[IRQ_OFFSET + 0] (IDT[32]) 在内核中包含时钟中断处理程序的地址。使用IRQ_OFFSET，以便设备中断不会与处理器异常重叠(事实上，在运行MS-DOS的PCs的早期，IRQ_OFFSET是零，这在硬件中断和处理器异常之间造成了巨大的混淆)。

## 3.2 进程间通信

最后要完成进程间通信(Inter-Process communication IPC)，常见的一个IPC例子就是管道。实现IPC有很多方式，哪种方式最好至今仍有争论，JOS中会实现一种简单的IPC机制。需要完成 `sys_ipc_try_send()` 和 `sys_ipc_recv()` 两个系统调用，以及封装了这两个系统调用的库函数实现。
